...
public:
    ...
    inline color_quad_u8_vec const& get_pixels() const { return m_pixels; }
    ...
    inline color_quad_u8 const& operator()(uint32_t x, uint32_t y) const
    {
        assert(x < m_width && y < m_height);
        return m_pixels[x + m_width * y];
    }
    ...
    image_u8& set_all(color_quad_u8 const& p)
    {
    ...
    image_u8& crop(uint32_t new_width, uint32_t new_height)
    {
        ...
        uint32_t const w = std::min(m_width, new_width);
        uint32_t const h = std::min(m_height, new_height);
    ...
    image_u8& swap(image_u8& other) noexcept
    {
    ...
    inline void set_block(uint32_t bx, uint32_t by, uint32_t width, uint32_t height, color_quad_u8 const* pPixels)
    {
        assert((bx * width + width) <= m_width);
        assert((by * height + height) <= m_height);
...
static bool load_png(char const* pFilename, image_u8& img)
{
    img.clear();
...
static bool save_png(char const* pFilename, image_u8 const& img, bool save_alpha)
{
    uint32_t const w = img.width();
    uint32_t const h = img.height();
...
public:
    ...
    void compute(image_u8 const& a, image_u8 const& b, uint32_t first_channel, uint32_t num_channels)
    {
        constexpr bool average_component_error = true;

        uint32_t const width = std::min(a.width(), b.width());
        uint32_t const height = std::min(a.height(), b.height());
        ...
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                color_quad_u8 const& ca = a(x, y);
                color_quad_u8 const& cb = b(x, y);
...
static bool save_dds(char const* pFilename, uint32_t width, uint32_t height, void const* pBlocks, uint32_t pixel_format_bpp, DXGI_FORMAT dxgi_format, bool srgb, bool force_dx10_header)
{
    (void)srgb;
    ...
    if ((!force_dx10_header) && ((dxgi_format == DXGI_FORMAT_BC1_UNORM) || (dxgi_format == DXGI_FORMAT_BC3_UNORM) || (dxgi_format == DXGI_FORMAT_BC4_UNORM) || (dxgi_format == DXGI_FORMAT_BC5_UNORM))) {
...
int main(int argc, char* argv[])
{
    ...
    uint32_t const bytes_per_block = (16 * pixel_format_bpp) / 8;
    ...
    if (src_alpha_filename.size()) {
        ...
        uint32_t const w = std::min(source_alpha_image.width(), source_image.width());
        uint32_t const h = std::min(source_alpha_image.height(), source_image.height());
    ...
    uint32_t const orig_height = source_image.height();
    ...
    uint32_t const blocks_x = source_image.width() / 4;
    uint32_t const blocks_y = source_image.height() / 4;
    ...
    if ((!no_output_png) && (png_output_filename.size())) {
        ...
        for (uint32_t by = 0; by < blocks_y; by++) {
            for (uint32_t bx = 0; bx < blocks_x; bx++) {
                ...
                case DXGI_FORMAT_BC5_UNORM:
                    ...
                    break;
                case DXGI_FORMAT_BC7_UNORM:
                    bc7decomp::unpack_bc7((uint8_t const*)pBlock, (bc7decomp::color_rgba*)unpacked_pixels);
                    break;
                default:
                    assert(0);
                    break;
        ...
        for (uint32_t chan = 0; chan < 4; chan++) {
            ...
            static char const* s_chan_names[4] = { "Red  ", "Green", "Blue ", "Alpha" };
