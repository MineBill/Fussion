...
package("slang", function()
    ...
    on_install("windows|x64", "linux|x86_64", function (package)
        ...
        if package:is_plat("windows") then
            os.cp("bin/slang.dll", package:installdir("bin"))
            os.cp("lib/slang.lib", package:installdir("lib"))
            -- Only shared is supported
        elseif package:is_plat("linux") then
            os.cp("lib/libslang.so", package:installdir("bin"))
            -- Only shared is supported
        end
    end)
