...
QueryIterator::QueryIterator(TSNode const& node, TSQuery const* query)
...
Query::Query(TSLanguage const* language, std::string const& query_source)
{
...

    {
        ...
        m_query = ts_query_new(language, query_source.c_str(), static_cast<uint32_t>(query_source.size()), &error_offset, &error_type);
    }
