...
namespace Reflect
{
    ...
    {
        if (Util::ToLower(str) == "private") return Private;
        if (Util::ToLower(str) == "public") return Public;
        if (Util::ToLower(str) == "friend") return Friend;
        return Invalid;
    }
    ...
    const char* ReflectReturnCodeToString(const EReflectReturnCode& code)
    {
        ...
        case EReflectReturnCode::FUNCTION_INVALID_RETURN_TYPE: return
                "ReflectFuncReturnCode - Function Invalid Return Type";

        default: throw std::runtime_error(
                "[ReflectFuncReturnCodeToString] Missing ReflectFuncReturnCode to string conversion.");
        }
    }
}