...
namespace Fussion::GPU {
    ...
    struct GPUHandle {
        ...
        GPUHandle() = default;
        explicit GPUHandle(HandleT handle_)
            : handle(handle_)
        {
        }
        GPUHandle(HandleT handle_, SpecT const& spec_)
            : handle(handle_)
            , spec(spec_)
        {
        }
    ...
    struct SamplerSpec {
        ...
        Maybe<CompareFunction> compare_fn {};
    ...
    struct Buffer final : GPUHandle<BufferSpec> {
        ...
        virtual void release() override;
        void force_set_map_state(MapState state);

        friend BufferSlice;

    private:
        MapState m_current_map_state { MapState::Unmapped };
    };
    ...
    struct ShaderModuleSpec {
        ...
        std::string_view vertex_entry_point_name {};
        std::string_view fragment_entry_point_name {};
    };
    ...
    struct PrimitiveState {
        ...
        static auto default_() -> PrimitiveState;
    };
    ...
    struct DepthStencilState {
        ...
        static auto default_() -> DepthStencilState;
    };
    ...
    struct BlendState {
        ...
        static auto default_() -> BlendState;
    };
    ...
    struct MultiSampleState {
        ...
        static auto default_() -> MultiSampleState;
    };
    ...
    struct RenderPipelineSpec {
        ...
        Maybe<std::string_view> vertex_entry_point_override {};
        /// Used if the shader module is a SpirV binary.
        Maybe<std::string_view> fragment_entry_point_override {};
        // MutliView
        // Cache
    };
    ...
    struct RenderPipeline : GPUHandle<void> {
        ...
        auto bind_group_layout_for(u32 index) -> BindGroupLayout;
        virtual void release() override;
    };
    ...
    struct Device final : GPUHandle<void> {
        ...
        [[nodiscard]] auto create_bind_group(BindGroupLayout layout, BindGroupSpec const& spec) const -> BindGroup;
...
#ifdef WGPU_DEV
        [[nodiscard]] auto create_shader_module_spir_v(SpirVShaderSpec const& spec) const -> ShaderModule;
#endif
        ...
        template<typename T>
        void write_buffer(Buffer const& buffer, u64 offset, ReadOnlySpan<T> data) const
        {
            write_buffer(buffer, offset, data.data(), data.size_in_bytes());
        }

        template<typename T>
        void write_buffer(Buffer const& buffer, u64 offset, std::vector<T> const& data) const
        {
            write_buffer(buffer, offset, data.data(), data.size() * sizeof(T));
        }
    ...
    struct Adapter {
        ...
        auto request_device(DeviceSpec const& spec) -> Device;
    ...
    struct Surface {
        ...
        struct Capabilities {
            std::vector<PresentMode> available_present_modes {};
            std::vector<TextureFormat> available_surface_formats {};
        };
        ...
        Capabilities capabilities(Adapter adapter) const;
        void configure(Device const& device, TextureFormat surface_format, Config const& config);

        [[nodiscard]] auto next_view() const -> Result<TextureView, Error>;
        void present() const;
    };
    ...
    struct Instance {
        ...
        auto request_adapter(Surface const& surface, AdapterOptions const& opt = {}) const -> Adapter;
