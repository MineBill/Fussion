...
namespace Fussion {
    ...
    auto ColorConstruct(Color* self)
    {
        new(self) Color();
    };

    auto ColorCopyConstructor(Color const& other, Color* self)
    {
        new(self) Color(other);
    };

    auto ColorInitConstructor(float r, float g, float b, float a, Vector3* self)
    {
    ...
    void ScriptingEngine::RegisterTypes()
    {
...

        {
            ...
            auto& metadata = keys_type.get_metadata();
            auto name = metadata.at("Name").as<std::string>();
            LOG_DEBUGF("Registering enum {}", name);
            r = m_ScriptEngine->RegisterEnum(name.data()); VERIFY(r >= 0);
...

        {
            ...
            r = m_ScriptEngine->RegisterGlobalFunction("bool IsAnyKeyDown(const Keys[] &in key)", asFUNCTION(CAST(bool(*)(CScriptArray*), is_any_key_down)), asCALL_CDECL); VERIFY(r >= 0);
        }
        ...
        r = m_ScriptEngine->RegisterObjectBehaviour("Color", asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(ColorConstruct), asCALL_CDECL_OBJLAST); VERIFY(r >= 0);
        r = m_ScriptEngine->RegisterObjectBehaviour("Color", asBEHAVE_CONSTRUCT, "void f(const Color &in)", asFUNCTION(ColorCopyConstructor), asCALL_CDECL_OBJLAST); VERIFY(r >= 0);
        r = m_ScriptEngine->RegisterObjectBehaviour("Color", asBEHAVE_CONSTRUCT, "void f(float r, float g = 0, float b = 0, float a = 0)", asFUNCTION(ColorInitConstructor), asCALL_CDECL_OBJLAST); VERIFY(r >= 0);
