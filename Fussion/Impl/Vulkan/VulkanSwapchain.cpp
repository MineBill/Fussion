...
namespace Fussion::RHI {
    ...
    VulkanSwapchain::VulkanSwapchain(VulkanDevice* device, Ref<RenderPass> render_pass, SwapChainSpecification const& spec)
        : m_Specification(spec), m_RenderPass(std::dynamic_pointer_cast<VulkanRenderPass>(std::move(render_pass)))
    {
        auto cap = device->SwapchainSupport.Capabilities;
    ...
    auto VulkanSwapchain::GetNextImage() -> Maybe<u32>
    {
        ZoneScoped;
    ...
    void VulkanSwapchain::Resize(u32 width, u32 height)
    {
        ZoneScoped;
        Destroy(true);
        m_Specification.Size = Vector2(width, height);
    ...
    void VulkanSwapchain::Destroy([[maybe_unused]] bool keep_handle)
    {
        ZoneScoped;
