...
namespace Reflect::Parser
{
    ...
    time_t GetFileTime(const std::filesystem::file_time_type time)
    {
    ...
    namespace
    {
        std::unordered_map<std::string, time_t> g_FileTimeStamps{};
    }

    void Cache::Open(std::filesystem::path file_path)
    {
        ...
        while (std::getline(file, line)) {
            ...
            if (pos != std::string::npos) {
                ...
                auto time = line.substr(pos + 1);

                g_FileTimeStamps[path] = std::stoll(time);
            }
        }
    }

    void Cache::Save(std::filesystem::path file_path)
    {
        ...
        for (const auto& [path, time] : g_FileTimeStamps) {
            file << path << "=" << time << '\n';
        }
    ...
    void Cache::SaveFile(const std::filesystem::directory_entry& entry)
    {
        g_FileTimeStamps[entry.path().string()] = GetFileTime(entry.last_write_time());
    }

    time_t Cache::GetEntry(const std::filesystem::directory_entry& entry)
    {
        const auto path = entry.path().string();
        if (g_FileTimeStamps.contains(path)) {
            return g_FileTimeStamps.at(path);
        }
        return 0;
    }
    ...
    void FileParser::ParseDirectory(const std::string& directory, const ReflectAdditionalOptions* options,
                                    FileParserOptions fileParserOptions)
    {
        ...
        for (const auto& f : std::filesystem::recursive_directory_iterator(directory)) {
            ...
            if ((f.is_regular_file() || f.is_character_file()) &&
                ...
                data.FileName = f.path().filename().string().substr(0, f.path().filename().string().find_last_of('.'));
    ...
    bool FileParser::CheckModificationTime(const std::filesystem::directory_entry& entry)
    {
        return GetFileTime(entry.last_write_time()) > Cache::GetEntry(entry);
    }
    ...
    bool FileParser::ReflectContainerHeader(FileParsedData& fileData, const std::string& keyword,
                                            const EReflectType type)
    {
        REFLECT_PROFILE_FUNCTION();

        // Check if we can reflect this class/struct.
        size_t reflectStart = fileData.Data.find(keyword, fileData.Cursor);
        ...
        fileData.Cursor = reflectStart + keyword.length();
        containerData.MetaProps = ReflectMetaProps(fileData);
        containerData.ContainerProps = ReflectFlags(fileData);

        if (containerData.ReflectType == EReflectType::Class) {
            size_t newPos = fileData.Data.find("class", fileData.Cursor);
            ...
            size_t newPos = fileData.Data.find("struct", fileData.Cursor);
            if (newPos != std::string::npos) {
                fileData.Cursor = newPos;
                fileData.Cursor += 6;
            }
        ...
        while (fileData.Data.at(fileData.Cursor) != ':' && fileData.Data.at(fileData.Cursor) != '{' && fileData.Data.
            at(fileData.Cursor) != '\n') {
    ...
    void FileParser::ReflectContainer(FileParsedData& fileData)
    {
        ...
        size_t endOfContainerCursor = FindEndOfContainer(fileData);
        ...
        if (generatedBodyLine == std::string::npos) {
            std::cout << "[FileParser::ReflectContainer] 'REFLECT_GENERATED_BODY()' is missing from a'" << fileData.
                FileName << "'.\n";
            generatedBodyLine = 0;
        }
        else {
        ...
        while (true) {
            std::vector<MetaProp> reflectPropertyMetas;
            ...
            if (m_options->options.at(Keys::Reflect_CMD_Option_Reflect_Full_EXT) == "true") {
                ...
                if (IsWordReflectKey(word)) {
                    ...
                    if (word == Keys::ReflectPropertyKey) {
                        // Get the flags for the property
                        reflectPropertyMetas = ReflectMetaProps(fileData);
                ...
                if (refectType == EReflectType::Member) {
                    ...
                    conatinerData.Members.back().MetaProps = reflectPropertyMetas;
                }
                    ...
                    if (!CheckForConstructor(fileData, conatinerData, word)) {
                        ...
                        if (!funcData.Name.empty()) {
                            ...
                            conatinerData.Functions.back().MetaProps = reflectPropertyMetas;
                        }
                    }
                }
                //else
                ...
                size_t newCursor = fileData.Data.find(Keys::ReflectPropertyKey, fileData.Cursor);
                ...
                fileData.Cursor += strlen(Keys::ReflectPropertyKey);
                reflectPropertyMetas = ReflectMetaProps(fileData);
                reflectFlags = ReflectFlags(fileData);
    ...
    size_t FileParser::FindEndOfContainer(const FileParsedData& fileData)
    {
        REFLECT_PROFILE_FUNCTION();
        ...
        while (true) {
            ...
            if (!foundStartOfContainer) {
                ...
                if (symbols.size() == 0) {
                    ...
                    assert(
                        nextChar == ';' &&
                        "[FileParser::FindEndOfConatiner] Did not find ';' at end of the container.");
                    break;
                }
            }
#else
    ...
    std::vector<MetaProp> FileParser::ReflectMetaProps(FileParsedData fileData)
    {
        ...
        if (const size_t metaIdx = scopeString.find(Keys::MetaPropertyKey);
            ...
            for (size_t metaIdx = 0; metaIdx < metas.size(); ++metaIdx) {
                ...
                MetaProp newPropertyMeta(key.c_str(), value.c_str());
            ...
            for (auto& reflectedData : file.ReflectData) {
                for (auto& member : reflectedData.Members) {
                    if (member.IsTemplate) {
                        auto const* template_first = FindReflectContainerData(member.TemplateFirstType);
                        auto const* template_second = FindReflectContainerData(member.TemplateSecondType);

                        if (template_first) {
                            member.TemplateFirstType = template_first->NameWithNamespace;
                        }
                        if (template_second) {
                            member.TemplateSecondType = template_second->NameWithNamespace;
                        }
                    }
                    else {
                        auto const* data = FindReflectContainerData(member.Type);
                        if (!data) {
                            continue;
                        }
                        member.NameWithNamespace = data->NameWithNamespace;
                    }
                }
                for (auto& func : reflectedData.Functions) {
                    for (auto& arg : func.Parameters) {
                        auto const* data = FindReflectContainerData(arg.RawType);
                        if (!data) {
                            continue;
                        }
                        arg.NameWithNamespace = data->NameWithNamespace;
                    }

                    auto const* data = FindReflectContainerData(func.Type);
                    if (!data) {
                        continue;
                    }
                    func.NameWithNamespace = data->NameWithNamespace;
                }
            }
        }
    }
    ...
    void FileParser::LinkAllInheritances()
    {
        std::function<void(ReflectInheritanceData&)> linkInheritanceFunc =
            ...
            ReflectContainerData* inheritanceContainerData = FindReflectContainerData(inheritanceData.Name);
        ...
        for (auto& file : m_filesParsed) {
            for (auto& reflectedData : file.ReflectData) {
                ...
                for (auto& member : reflectedData.Members) {
                    ...
                    ReflectContainerData* memberInheritanceContainerData =
                        FindReflectContainerData(member.Type);
    ...
    void FileParser::RemoveLookupOnlyContainers()
    {
        for (size_t parsedDataIdx = 0; parsedDataIdx < m_filesParsed.size(); ++parsedDataIdx) {
            ...
            for (size_t refecltDataToRemoveIdx = 0; refecltDataToRemoveIdx < reflectDataToBeRemoved.size(); ++
                 refecltDataToRemoveIdx) {
                const ReflectContainerData* removeData = reflectDataToBeRemoved[refecltDataToRemoveIdx];
                auto removeIter = std::remove_if(parsedData.ReflectData.begin(), parsedData.ReflectData.end(),
                                                 [&removeData](const ReflectContainerData& reflectData) {
    ...
    char FileParser::FindNextChar(const FileParsedData& fileData, size_t& cursor, const std::vector<char>& ignoreChars,
                                  bool reverse)
    {
        REFLECT_PROFILE_FUNCTION();
    ...
    std::string FileParser::FindNextWord(const FileParsedData& fileData, size_t& cursor,
                                         const std::vector<char>& endChars, bool reverse)
    {
        FileParsedData copyFileData = fileData;
        copyFileData.Cursor = cursor;
    ...
    void FileParser::MoveToEndOfScope(FileParsedData& fileData, const char startScopeChar,
                                      const char endScopeChar) const
    {
        size_t scope = 1;
    ...
    bool FileParser::CheckForConstructor(FileParsedData& fileData, ReflectContainerData& container,
    ...
    void FileParser::GetReflectNameAndReflectValueTypeAndReflectModifer(std::string& str, std::string& name,
                                                                        EReflectValueType& valueType,
                                                                        EReflectValueModifier& modifer)
    {
        REFLECT_PROFILE_FUNCTION();
    ...
    ReflectFunctionData FileParser::GetFunction(FileParsedData& fileData, const std::vector<std::string>& flags)
    {
        ...
        uint32_t cBracket = static_cast<uint32_t>(line.find_last_of(')'));
        size_t functionConst = static_cast<uint32_t>(line.find(Keys::ConstKey, cBracket));
        if (functionConst != std::string::npos) {
            functionData.IsConst = true;
            line = line.substr(0, cBracket + 1);
        }
        uint32_t oBracket = static_cast<uint32_t>(line.find_first_of('('));
        ...
        uint32_t cursor = static_cast<uint32_t>(line.size()) - 1;
        ...
        GetReflectNameAndReflectValueTypeAndReflectModifer(line, functionData.Name, functionData.ReflectValueType,
                                                           functionData.ReflectModifier);
    ...
    ReflectMemberData FileParser::GetMember(FileParsedData& fileData, const std::vector<std::string>& flags)
    {
        ...
        size_t endOfLineCursor = fileData.Cursor + line.size();
        line += ';';

        // Check for if there is a deault value being set.
        uint32_t equalCursor = static_cast<uint32_t>(line.find('='));
        uint32_t semicolonCursor = static_cast<uint32_t>(line.find(';'));
        ...
        GetReflectNameAndReflectValueTypeAndReflectModifer(line, memberData.Name, memberData.ReflectValueType,
                                                           memberData.ReflectModifier);

        cursor = static_cast<uint32_t>(line.size()) - 1;
        while (std::find(emptyChars.begin(), emptyChars.end(), line.at(cursor)) != emptyChars.end())
            --cursor;
    ...
    EReflectType FileParser::CheckForReflectType(FileParsedData& data)
    {
        ...
        auto find_closest_char = [data, this](const std::vector<char>& chars_to_find) {
        ...
        if (function_cursor != INT_MAX && !isTemplate) {
    ...
    EReflectValueType FileParser::CheckForRefOrPtr(std::string_view view)
    {
        ...
        if (referenceIndex < pointerIndex)
            return EReflectValueType::Reference;
        if (pointerIndex < referenceIndex && referenceIndex == std::string::npos)
    ...
    EReflectValueModifier FileParser::CheckForMemberModifers(std::string_view view)
    {
        ...
        if (volatileIndex != std::string::npos)
            return EReflectValueModifier::Volatile;
        if (virtualIndex != std::string::npos)
    ...
    std::vector<ReflectTypeNameData> FileParser::ReflectGetFunctionParameters(
        const FileParsedData& fileData, std::string_view view)
    {
        REFLECT_PROFILE_FUNCTION();
        ...
        while (cursor < view.size()) {
            if (c != ',' && c != ')')
                ...
                if (str.size() > 0 && !Util::StringContains(str, emptyChars)) {
                    ...
                    GetReflectNameAndReflectValueTypeAndReflectModifer(str, parameter.Name, parameter.ReflectValueType,
                                                                       parameter.ReflectModifier);
        ...
        for (auto& file : m_filesParsed) {
            for (auto& reflectedData : file.ReflectData) {
                ...
                for (size_t i = 0; i < compareNames.size(); i++) {
                    ...
                    std::vector<std::string> reflectDataSplit = Util::SplitString(
                        reflectedData.NameWithNamespace, static_cast<char>('::'));
                    std::vector<std::string> containerNameSplit = Util::SplitString(
                        std::string(compareName), static_cast<char>('::'));

                    std::ranges::reverse(reflectDataSplit);
                    std::ranges::reverse(containerNameSplit);
