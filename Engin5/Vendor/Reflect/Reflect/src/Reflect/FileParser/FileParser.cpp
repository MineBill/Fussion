...
namespace Reflect::Parser
{
    ...
    void Cache::Save(std::filesystem::path file_path)
    {
        ...
        for (auto const& [path, time] : g_FileTimeStamps) {
            file << path << "=" << time << '\n';
        }
    ...
    void Cache::SaveFile(std::filesystem::directory_entry const& entry)
    {
        g_FileTimeStamps[entry.path().string()] = GetFileTime(entry.last_write_time());
    }

    time_t Cache::GetEntry(std::filesystem::directory_entry const& entry)
    {
	...
	void FileParser::ParseDirectory(const std::string& directory, const ReflectAdditionalOptions* options, FileParserOptions fileParserOptions)
	{
    ...
    bool FileParser::CheckModificationTime(std::filesystem::directory_entry const& entry)
    {
