...
#define WRITE_CURRENT_FILE_ID(ContainerName, FileName) file << "#define " + GetCurrentFileID(ContainerName, FileName)
    ...
    void CodeGenerateHeader::GenerateHeader(const Parser::FileParsedData& data, std::ofstream& file,
                                            const ReflectAdditionalOptions* additionalOptions)
    {
        REFLECT_PROFILE_FUNCTION();

        file <<
            " // This file is auto generated by the Reflect tool please don't modify this file as it will be overwritten.\n";
        ...
        file << "#error \"" + data.FileName + Keys::ReflectFileGeneratePrefix + ".h" +
            " already included, missing 'pragma once' in " + data.FileName + ".h\"\n";
    ...
    void CodeGenerateHeader::WriteMacros(const Parser::FileParsedData& data, std::ofstream& file,
                                         const ReflectAdditionalOptions* additionalOptions)
    {
        REFLECT_PROFILE_FUNCTION();
        for (const auto& reflectData : data.ReflectData) {
            ...
            const std::string CurrentFileId = GetCurrentFileID(reflectData.Name, data.FileName) + "_" + std::to_string(
                reflectData.ReflectGenerateBodyLine);
            ...
            WRITE_CURRENT_FILE_ID(reflectData.Name, data.FileName) + "_" + std::to_string(
                reflectData.ReflectGenerateBodyLine) + "_GENERATED_BODY \\\n";
