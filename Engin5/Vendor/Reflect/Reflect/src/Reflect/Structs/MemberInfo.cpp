...
namespace Reflect
{
    ...
    MemberInfo::MemberInfo(Type type, std::string memberName, EReflectValueType valueType,
        ...
        : Property(std::move(type), std::move(memberName), PropertyType::Member, std::move(flags),
                   std::move(propertyMetas), objectInstance)
          , m_valueType(valueType)
          , m_modifierType(modiferType)
          , m_memberOffset(offset)
    {
    ...
    void* MemberInfo::GetMemberPointer() const
    {
        ...
        auto objectInstance = static_cast<unsigned char*>(m_objectInstance);
