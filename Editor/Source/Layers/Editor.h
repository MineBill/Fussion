...
public:
    ...
    static AssetPicker s_generic_asset_picker;
    ...
    void create_asset_window(Fussion::AssetHandle handle, Args&&... args)
    {
        if (m_asset_windows.contains(handle))
            return;
        m_asset_windows[handle] = make_ptr<T>(handle, std::forward<Args>(args)...);
    }

    void set_play_state(PlayState new_state);
    auto play_state() const -> PlayState { return m_play_state; }
    ...
    static auto self() -> Editor& { return *s_editor_instance; }
    ...
    {
        if (s_editor_instance->m_play_state == PlayState::Playing) {
    ...
    static auto content_browser() -> ContentBrowserWindow& { return *s_editor_instance->m_content_browser.get(); }
...
private:
    ...
    Ptr<ContentBrowserWindow> m_content_browser;
    ...
    Ptr<EngineInfoWindow> m_engine_info_window;
